...
void SimpleSoft::RegisterPlugin() {
    ...
    plugin.nstate            = +[](const mjModel* m, int instance) { return 214 * 3 * 2; }; // return force number
...
void SimpleSoft::Compute(const mjModel* m, mjData* d, int instance) {
    ...
    if(compute_counter%sim_step!=0) return;
    ...
    if (robot_sdf_left && robot_sdf_right) {
        ...
        robot_pos_left3  = GetBodyPos("J_WRIST_L1");     // [robot]
        robot_pos_right1 = GetJointPos("J_SHOULDER_R2"); // [robot]
        robot_pos_right2 = GetJointPos("J_ELBOW_R1");    // [robot]
        robot_pos_right3 = GetBodyPos("J_WRIST_R1");     // [robot]
        cuVec3 pos1      = cuVec3(robot_pos_left1[0], robot_pos_left1[1], robot_pos_left1[2]);
        cuVec3 pos2      = cuVec3(robot_pos_left2[0], robot_pos_left2[1], robot_pos_left2[2]);
        cuVec3 pos3      = cuVec3(robot_pos_right1[0], robot_pos_right1[1], robot_pos_right1[2]);
        cuVec3 pos4      = cuVec3(robot_pos_right2[0], robot_pos_right2[1], robot_pos_right2[2]);
        UniX2MuJoCO(pos1);
        UniX2MuJoCO(pos2);
        UniX2MuJoCO(pos3);
        UniX2MuJoCO(pos4);
